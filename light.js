...
// ===YOUR CODE STARTS HERE===
		...
		let lv = shadingPoint.clone();
		lv.sub(this.from).normalize();
		let alpha = dir.dot(lv);
		let cutoff_angle = Math.cos((this.cutoff / 2) * (Math.PI / 180));
		if (alpha > cutoff_angle) {
			let ls = new LightSample();
			ls.position = this.from.clone();
			ls.direction = this.from.clone();
			ls.direction.sub(shadingPoint)
			ls.intensity = this.intensity.clone();
			ls.intensity.multiplyScalar(Math.pow(alpha, this.exponent)/ls.direction.lengthSq());
			ls.direction.normalize();
			return ls;
		} else {
			return null;
